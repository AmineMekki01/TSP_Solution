...
def brute_force(dist_matrix):
    ...
    for i in range(len(best_route) - 1):
        result.append((best_route[i], best_route[i + 1], int(distance(best_route[i], best_route[i + 1], dist_matrix))))
    result.append((best_route[-1], best_route[0], int(distance(best_route[-1], best_route[0], dist_matrix))))
